...
public class FUNCIONAOSTIA {
	...
	public FUNCIONAOSTIA (int places_no_discapacitats, int places_discapacitats) {
			...
			if(matricula.matches("[0-9]{4}[A-Z]{3}")) {
				...
				if( !(plazasNormales.containsKey(matricula)) && !(plazasDisc.containsKey(matricula)) ) {
					...
					if(contadorPlazasDisc >= 1 && (int)(Math.random()*10) == 8) {
						...
						if(avisoOcupacionDisc() == false) {
							System.out.println("ALERTA =====> Ocupació de places per discapacitats supera el 85%.");
						}

						System.out.println("ALERTA =====> Garrulo detected!!! Ha aparcat a la plaça: " + plazaParking);
				...
				} else System.out.println("El cotxe ja està al parking. No pot entrar.");
			...
			if(matricula.matches("[0-9]{4}[A-Z]{3}")) {
				...
				if( !(plazasDisc.containsKey(matricula)) ) {
					...
					if(contadorPlazasDisc >= 1) {
						...
						System.out.println("ALERTA =====> Parking per discapacitats ple. Ha ocupat plaça normal num: " + plazaParking);
	...
	/**
	 * Metodo utilizado en el constructor para inicializar las boolean en False
	 */
	private void iniciarBooleansFalse() {

		for(int i = 0;i < booleanPlazasNormales.length;i++) {

			booleanPlazasNormales[i] = false;

		}

		for(int i = 0;i <booleanPlazasDisc.length;i++) {

			booleanPlazasDisc[i] = false;
	...
	private boolean avisoOcupacionNormal() {
		...
		if(((contadorPlazasNormales*100)/places_no_discapacitats)>85){
			return true;
		}
		else return false;
	}
	...
	private boolean avisoOcupacionDisc() {
		...
		if(((contadorPlazasDisc*100)/places_discapacitats)>85){
			return true;
		}
		else return false;
	}
